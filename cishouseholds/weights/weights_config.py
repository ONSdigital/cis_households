...
def operation2(country_check: Any, age_group_antibody_column, *args, **logic):
    return (
        ...
        .when(country_check, logic["second"])
        .otherwise(logic["otherwise"])
    )
...
def wrapper(df: DataFrame, country_column: str, age_group_antibody_column: str):
    operations = {1: operation1, 2: operation2}
    for column_name, logic in dictionary.items():
        ...
        df = df.withColumn(
            column_name,
            inner_function(F.col(country_column).isin(logic["country"]), age_group_antibody_column, **logic),
        )
...
dictionary = {
    ...
    "p1_for_antibodies_evernever_engl": {
        "operation": 1,
        "country": "England",
        "if_country": ((F.col("interim_region_code") - 1) * 10)
        + ((F.col("interim_sex") - 1) * 5)
        + F.col("age_group_antibody"),
        "otherwise": None,
    },
