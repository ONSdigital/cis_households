...
def all_columns_null(column_list: List[str]):
    "Expression that evaluates true if all columns are null."
    return reduce(and_, [F.col(column).isNull() for column in column_list], F.lit(True))


def any_column_null(column_list: List[str]):
    "Expression that evaluates true if any column is null."
    return reduce(or_, [F.col(column).isNull() for column in column_list], F.lit(True))
...
def all_equal(column_list: List[str], equal_to: Any):
    ...
    return reduce(and_, [F.col(column).eqNullSafe(F.lit(equal_to)) for column in column_list], F.lit(True))
