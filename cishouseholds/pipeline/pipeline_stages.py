...
def merge_swab_ETL_a(
    ...
    swab_output_tables_xtox_list: List[str],
):
    # common
    survey_df = extract_from_table(survey_responses_table).where(
        F.col("unique_participant_response_id").isNotNull() & (F.col("unique_participant_response_id") != "")
    ...
    # common
    load_to_data_warehouse_tables(output_swab_df_list, swab_output_tables_xtox_list)
...
def merge_swab_ETL_b(
    ...
    for table_name, table in zip(xtox_list, swab_input_tables_xtox_list):
        table_dict_xtox[table_name] = extract_from_table(table)
    ...
    one_to_many_df = one_to_many_swabs(
        df=table_dict_xtox["one_to_many_df"],
    ...
    update_table(df=one_to_many_df, table_name="one_to_many_table_2", mode_overide="overwrite")

    many_to_one_df = many_to_one_swab_flag(
        df=table_dict_xtox["many_to_one_df"],
        column_name_to_assign="drop_flag_mto1_" + merge_type,
        group_by_column="swab_sample_barcode",
        ordering_columns=window_columns,
    )
    update_table(df=many_to_one_df, table_name="many_to_one_table_2", mode_overide="overwrite")

    many_to_many_df = many_to_many_flag(
        df=table_dict_xtox["many_to_many_df"],
    ...
    update_table(df=many_to_many_df, table_name="many_to_many_table_2", mode_overide="overwrite")


@register_pipeline_stage("merge_swab_ETL_c")  # STAGE C: union_multiple_tables, merge_process_validation, null_safe_join
def merge_swab_ETL_c(
    swab_input_tables_xtox_list,
):
    table_dict_xtox = {}
    xtox_list = [
    ...
    unioned_df = union_multiple_tables(tables=[table_dict_xtox[table] for table in xtox_list[:-1]])
    unioned_df = merge_process_validation(
        df=unioned_df,
        merge_type=merge_type,
        barcode_column_name="swab_sample_barcode",
    )
    unioned_df = null_safe_join(
        left_df=unioned_df,
        right_df=table_dict_xtox["df_non_specific_merge"],
        null_safe_on=["unique_pcr_test_id"],
        null_unsafe_on=["unique_participant_response_id"],
        how="left",
    )
    # unioned_df = unioned_df.drop(
    #     "abs_offset_diff_vs_visit_hr_swab",
    #     "count_barcode_swab",
    #     "count_barcode_voyager",
    #     "diff_vs_visit_hr_swab",
    ...
    # )
    update_table(df=unioned_df, table_name="unioned_table", mode_overide="overwrite")


@register_pipeline_stage("merge_swab_ETL_d")  # STAGE D: union_multiple_tables
def merge_swab_ETL_d(input_table, none_record_table, swab_output_tables):

    df = extract_from_table(input_table)
    none_record_df = extract_from_table(none_record_table)
