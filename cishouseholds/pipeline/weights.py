...
def proccess_population_projection_df(previous_projection_df: DataFrame, current_projection_df: DataFrame, month: int):
    ...
    current_projection_df.toPandas().to_csv("reformatted_table.csv", index=False)
    ...
    england_df, wales_df, scotland_df, ni_df, england_28_df = get_calibration_dfs(current_projection_df, "country_name_#", "age")
...
def get_calibration_dfs(df: DataFrame, country_column: str, age_column: str):
    ...
    def calibarate_df(df: DataFrame, groupby_columns: List[str], country: str, min_age: int, additional_columns: List[str]=[]):
        df = df.filter(
            (F.col(country_column) != country) & (F.col(age_column) >= min_age) & (F.col("p1_for_swab_longcovid").isNotNull())
        ).select(*groupby_columns, "population", *additional_columns)
        for col in groupby_columns:
            window = Window.partitionBy(*groupby_columns)
            df = df.withColumn(f"{col[:2]}_sum_population", F.sum("population").over(window))
        if len(groupby_columns) == 1:
            df = df.groupBy(groupby_columns[0]).pivot(f"{groupby_columns[0][:2]}_sum_population").count()
        else:
            df = df.withColumn("TEMP_ARRAY", F.array(*[F.col(col) for col in groupby_columns]))
            df = df.groupBy(*groupby_columns).pivot("TEMP_ARRAY").count()
            df.toPandas().to_csv("TEMPTEST.csv", index=False)
        print("SHOWING>>>")
        df.show()
        return df

    england_df = calibarate_df(df=df, groupby_columns=["p1_for_swab_longcovid"], country="England", min_age=2)
    wales_df = calibarate_df(df=df, groupby_columns=["p1_for_antibodies_wales_scot_ni"], country="Wales", min_age=16, additional_columns=["p22_white_population_antibodies"])
    scotland_df = calibarate_df(df=df, groupby_columns=["p1_for_antibodies_wales_scot_ni"], min_age=16, country="Scotland")
    ni_df = calibarate_df(df=df, groupby_columns=["p1_for_antibodies_wales_scot_ni"], min_age=16, country="Northern Ireland")

    england_28_df = calibarate_df(df=df,groupby_columns=["p1_for_antibodies_28daysto_engl"],country="England", min_age=16, additional_columns=["p3_for_antibodies_28daysto_engl","population","p22_white_population_antibodies"])
