...
def assign_group_and_row_number_columns(
    ...
    df = df.join(F.broadcast(dft), dft.b == F.col(group_by_column)).drop("b", "dummy")
    return df
...
def flag_rows_different_to_reference_row(
    ...
    df = df_reference_row_and_col_only.join(
        F.broadcast(df.withColumnRenamed(reference_column, reference_column + "_reference")),
        on=[group_column],
        how="inner",
    )
...
def check_consistency_in_retained_rows(
    ...
    for col in check_columns:
        ...
        df = df.join(F.broadcast(df_grouped), on=[group_column], how="inner")
